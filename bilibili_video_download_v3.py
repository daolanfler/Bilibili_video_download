    ...
    for title in title_list:
        ...
        if len(os.listdir(current_video_path)) >= 2:
            ...
            final_clip.to_videofile(
                os.path.join(current_video_path, r"{}.mp4".format(title)),
                fps=24,
                remove_temp=False,
            )
            print("[视频合并完成]" + title)
...
if __name__ == "__main__":
    ...
    for item in cid_list:
        ...
        # 定义线程
        th = threading.Thread(
            target=down_video, args=(video_list, title, start_url, page)
        )
        # 将线程加入线程池
    ...
    print(
        "下载总耗时%.2f秒,约%.2f分钟" % (end_time - start_time, int(end_time - start_time) / 60)
    )
