        ...
        if len(video_list) > 1:
            urllib.request.urlretrieve(
                ...
                filename=os.path.join(
                    currentVideoPath, r"{}-{}.mp4".format(title, num)
                ),
            ...
            urllib.request.urlretrieve(
                url=i,
                filename=os.path.join(currentVideoPath, r"{}.mp4".format(title)),
                reporthook=Schedule_cmd,
            )  # 写成mp4也行  title + '-' + num + '.flv'
        num += 1
...
def combine_video(video_list, title):
    ...
    if len(video_list) >= 2:
        ...
        final_clip.to_videofile(
            os.path.join(root_dir, r"{}.mp4".format(title)), fps=24, remove_temp=False
        )
        print("[视频合并完成]" + title)
...
if __name__ == "__main__":
    ...
    if start.startswith("BV") == True:  # 如果输入的是av号
        # 获取cid的api, 传入bvid即可
        ...
        start_url = "https://api.bilibili.com/x/web-interface/view?bvid=" + re.search(
            r"/BV([a-z0-9A-Z]+)/*", start
        ).group(1)
